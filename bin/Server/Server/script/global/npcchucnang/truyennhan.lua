...
function main(NpcIndex)
	...
	Say("LINK:<npc>",random(0,2),11312,6,
...
function nhanpd()
	if(IsFuYuanAvailable() ~= 1) then
		FuYuan_Start();
		FuYuan_Add(2);
		Say("Ng­¬i míi s¬ nhËp giang hå, ®©y lµ mét lÔ vËt nhá, h·y nhËn lÊy.",0);
	return end;
...
if (GetGameTime() - GetTask(TASK_THOIGIAN9) >= 43200) then
	...
	Say("Ng­¬i vÉn ch­a ®ñ ®iÒu kiÖn ®Ó nhËn danh väng. H·y cè g¾ng h¬n n÷a!",0)
end
end;
...
function tichluy()
	...
	Say(format("KÓ tõ lÇn nhËn phóc duyªn tr­íc, ng­¬i ®· tİch lòy <color=Yellow>%d g %d phót %d gi©y<color> thêi gian Online",nFYValidTime/60/60/18,mod(nFYValidTime/60/18,60),mod(nFYValidTime/18,60)));
end;
