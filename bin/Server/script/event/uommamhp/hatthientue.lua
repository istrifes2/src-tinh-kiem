...
function main(nItemIdx)
	...
	--if (CheckTime(now) == 1) then
		if (CheckLimit() == 1) then
			...
			local nNpcIndex = AddNpcNew(THIENTUE_TEMPLET[nRand][1],1,nW,nX*32,nY*32,SCRIPT_TREEDLG,6);
			if (nNpcIndex <= 0) then
				return 0;
			else
				if (nRand == 6) then
				...
				SetNpcTimeout(nNpcIndex, TREE_TIME_LIMIT); -- 15 phut sau tu mat
				SetNpcName(nNpcIndex,nName..THIENTUE_TEMPLET[nRand][2]);
				SetNpcOwner(nNpcIndex,GetName(),0,0);
				SetNpcParam(nNpcIndex,0,RANDOM(1,BASE_SCORE_RATE));
				SetNpcParam(nNpcIndex,1,RANDOMC(300,312,324,356,368,372,374,314,316,318,320,322,324,326,328,330));
...
function CheckLimit()
	...
	if (PermitRegion() ~= 1 or CheckMap(SubWorldIdx2ID(SubWorld)) ~= 1 or GetFightState() ~= 0) then
