...
KProtocolProcess::KProtocolProcess()
{
	...
	ProcessFunc[s2c_playermissiondata] = s2cSetMissionData;
	...
	ProcessFunc[s2c_syncmagic] = s2cSyncMagic;
	...
	ProcessFunc[s2c_npcstand] = NetCommandStand;
	ProcessFunc[s2c_npcchat] = NetCommandChat;
	ProcessFunc[s2c_npcsetpos] = NetCommandSetPos;
	...
	ProcessFunc[s2c_ignorestate] = IgnoreState;
	ProcessFunc[s2c_opensomebox] = OpenSomeBox;
	...
	ProcessFunc[s2c_createtong] = s2cCreateTong;
	ProcessFunc[s2c_jointong] = s2cJoinTong;
	ProcessFunc[s2c_jointongreply] = s2cJoinTongReply;
	...
	ProcessFunc[s2c_playertradeaction] = SendTradeAction;
	ProcessFunc[s2c_playertradeitem] = SendTradeItem;
	ProcessFunc[s2c_playertradecount] = SendTradeCount;	
	ProcessFunc[s2c_playertradefail] = SendTradeFail;	
	ProcessFunc[s2c_titlename] = NetCommandSetRank;
	ProcessFunc[s2c_expandrank] = NetCommandSetExpandRank;
	ProcessFunc[s2c_npchorsesync] = NetCommandSetHorse;
	ProcessFunc[s2c_synctaskvalue] = s2cSyncTaskValue;
	ProcessFunc[s2c_playersync] = s2cPlayerSync;
	ProcessFunc[s2c_extpoint] = SetExtPoint;
	ProcessFunc[s2c_opengive] = SyncGive;
	ProcessFunc[s2c_syncmasklock] = SyncMaskLock;
	ProcessFunc[s2c_syncrankdata] = s2cSyncRankData;
	ProcessFunc[s2c_syncsupershop] = s2cSyncSuperShop;
	ProcessFunc[s2c_sparapplystart] = s2cSparApplyStart;
	ProcessFunc[s2c_lockmove] = s2cLockMove;
	ProcessFunc[s2c_chatroomstate] = s2cChatRoomState;
	ProcessFunc[s2c_chatroomcreatefalse] = s2cApplyCreateChatRoomFalse;
	ProcessFunc[s2c_requestchatroomlist] = s2cRequestChatRoomList;
	ProcessFunc[s2c_requestmemberlist] = s2cRequestMemberList;
	ProcessFunc[s2c_requestblacklist] = s2cRequestBlackList;
