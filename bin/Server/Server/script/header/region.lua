--hoang
NUM_OFFLINE_PERMITREGION_MAX = 10;
...
function PermitRegion()
	local nMapId, nX, nY = GetWorldPos();
	local nLimitX, nLimitY = 0,0;
	local szDirection, nDistance = "", 0;
	local i=0;
	if LIMIT_REGION[nMapId] then
		for i=1, getn(LIMIT_REGION[nMapId]) do
			nLimitX = LIMIT_REGION[nMapId][i][1];
			nLimitY = LIMIT_REGION[nMapId][i][2];
