...
for i=1,dropnum do
	...
	elseif(nSel == 9) then
		...
		DropItem(NpcIndex, AddItem(0,0,bluedetail,blueparti,1,ser,pluck,
...
local parti = RANDOM(ITEMLIST[detail+1]);
	...
	elseif(nSel == 5) then
		if(random(2) == 1) then
		nItemIdx = AddItem(0,6,RANDOMC(205,391),0,0,5,0,0);
		SetObjPickExecute(DropItem(NpcIndex, nItemIdx),1)	
		end;
	else
		...
		DropItem(NpcIndex, AddItem(0,0,detail,parti,1,ser,pluck,
